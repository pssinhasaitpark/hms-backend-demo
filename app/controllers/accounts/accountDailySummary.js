...
export const getMonthlyAttendance = async (req, res) => {
  try {
    ...
    //   patientType: { $in: ["NEW", "FOLLOWUP", "PREPAID"] },
    // }).select("serviceCharge totalFee patientType");

    // const patientRevenue = patientDocs.reduce(
    //   (sum, p) => sum + (p.serviceCharge || 0),
    //   0
    // );

    // ---------------------- SERVICE CHARGE ONLY REVENUE ----------------------
    const visitDocs = await PatientVisitRecords.find({
      doctor: doctorId,
      visitDate: { $gte: startDate, $lte: endDate },
    }).select("serviceCharge");

    const totalRevenue = visitDocs.reduce(
    ...
    // const totalRevenue = patientRevenue + invoiceRevenue;

    // Calculate doctor's salary
...
//         res,
//         400,
//         "Invalid status. Only 'received' status is allowed."
//       );
//     }
...
//         res,
//         400,
//         "Transaction status is not 'submitted', cannot update."
//       );
//     }
...
/* export const updateTransactionStatus = async (req, res) => {
  try {
    ...
    if (accountSummary.transactionStatus === "declined" && transactionStatus === "received") {
      return handleResponse(
        res,
        400,
    ...
    if (accountSummary.transactionStatus === "received" && transactionStatus === "declined") {
      return handleResponse(
        res,
        400,
    ...
    accountSummary.transactionStatus = transactionStatus;
...
/* export const updateTransactionStatus = async (req, res) => {
  try {
    ...
    // Block changing from declined → received
    if (accountSummary.transactionStatus === "declined" && transactionStatus === "received") {
      return handleResponse(
        res,
        400,
    ...
    // Block changing from received → declined directly
    if (accountSummary.transactionStatus === "received" && transactionStatus === "declined") {
      return handleResponse(
        res,
        400,
    ...
    // If trying to set declined, mark as pending instead
    if (transactionStatus === "declined") {
      accountSummary.transactionStatus = "pending";
    } else {
      accountSummary.transactionStatus = transactionStatus;
    ...
    // Update receivedAt only if received
    if (transactionStatus === "received") {
      accountSummary.receivedAt = new Date();
    } else {
      accountSummary.receivedAt = null;
