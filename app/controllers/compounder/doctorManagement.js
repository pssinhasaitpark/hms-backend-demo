  ...
  try {
    ...
    const doctorsWithDetails = await Promise.all(
      paginatedDoctors.map(async (doctor) => {
        ...
        const lastCheckInCheckout =
          checkRecords.find((rec, index) => {
            return index !== 0 && rec.status === "CheckedOut";
          }) || null;
...
/* export const getCompounderAssignedPatients = async (req, res) => {
    try {
      ...
      // Fetch compounder with assigned doctors
      const compounder = await Compounder.findOne({ compounderId })
        .populate("doctors", "_id doctorName doctorId")
        .lean();
      ...
      const doctorIds = compounder.doctors.map((d) => d._id);
      const { search, date } = req.query;
      const { page, limit, skip } = getPagination(req);

      // 1️⃣ ALWAYS FETCH ALL ASSIGNED PATIENTS FIRST (NO DATE FILTER)
      let allPatients = await PatientVisitRecords.find({
        doctor: { $in: doctorIds },
        appointmentStatus: { $ne: "COMPLETED" }
      })
        .populate("doctor", "doctorName doctorId")
        .populate("department", "name")
        .populate("patient", "patientName mobile gender age address")
        .sort({ visitDate: 1 })
        .lean();

      console.log("ALL ASSIGNED PATIENTS COUNT:", allPatients.length);
      ...
      // 2️⃣ APPLY DATE FILTER ONLY IF DATE PROVIDED
      if (date) {
        ...
        // If date filter returns ZERO patients → return all patients instead
        if (filteredPatients.length === 0) {
          console.log("⚠️ No patients found for this date, returning ALL patients");
      ...
      if (search) {
        filteredPatients = filteredPatients.filter((p) =>
          (p.patientSnapshot?.patientName || "")
            .toLowerCase()
            .includes(search.toLowerCase()) ||
          (p.patientSnapshot?.mobile || "").includes(search) ||
          (p.visitId || "").includes(search)
        );
      }
      ...
      return handleResponse(res, 200, "Patients fetched successfully", {
        ...
        limit,
        totalPages: Math.ceil(totalPatients / limit),
        assignedDoctorsCount: doctorIds.length,
        assignedDoctors: compounder.doctors,
        patients: paginated
      });
