...
/* async function calculatePrepaiServicesTotal(prepaidServices, doctorId) {
  let totalFee = 0;
  let serviceEntries = [];
  let serviceSnapshot = [];

  if (!prepaidServices?.length)
    return { totalFee, serviceEntries, serviceSnapshot };

  const serviceIds = prepaidServices.map((s) => s.serviceId);
  ...
  serviceDocs.forEach((s) => {
    const prepaid = prepaidServices.find(
      (p) => p.serviceId === s._id.toString()
    );
    const quantity = prepaid?.quantity || 1;
    const totalPrice = s.charge * quantity;
...
export const getOrCreateMobile = async (phone, frontDeskId) => {
  ...
  return mobile; // This will always be a valid mobile document
};
...
/* async function handleNewPatient(req, res, frontDeskId, mobile) {
  ...
  const regFeeDoc = await RegistrationFee.findOne().sort({ updatedAt: -1 });
  const registrationFee = regFeeDoc?.registrationFee || 50;

  const opdFeeDoc = await OpdRegistrationFee.findOne().sort({ updatedAt: -1 });
  const opdCharge = opdFeeDoc?.registrationFee || 50;
  ...
  // if (paymentType === "CASH")
  //   await updateAccountDailySummary(frontDeskId, totalFee, 0);
  // else if (paymentType === "ONLINE")
  //   await updateAccountDailySummary(frontDeskId, 0, totalFee);

  if (paymentType === "CASH") {
    await updateAccountDailySummary(frontDeskId, totalFee, 0, 0, 0, 0, 0);
  } else {
    await updateAccountDailySummary(frontDeskId, 0, totalFee, 0, 0, 0, 0);
  }
...
/* async function handleFollowupPatient(req, res, frontDeskId, mobile) {
  ...
  const lastVisitRecord = await PatientVisitRecords.findOne({
    patient: patient._id,
    doctor: doctor._id,
  })
    .sort({ visitDate: -1 })
  ...
  if (lastVisitRecord) {
    ...
    //   (new Date() - new Date(lastVisitRecord.visitDate)) / (1000 * 60)
    // );
    ...
    const followupWindowMinutes = 8 * 24 * 60;

    if (minutesDiff <= followupWindowMinutes) {
      patientTypeForVisit = "FOLLOWUP";
      opdCharge = 0;
    } else {
      patientTypeForVisit = "REGULAR";
      opdCharge = departmentOpdCharge;
    }
  ...
  if (paymentType === "CASH") {
    await updateAccountDailySummary(frontDeskId, finalTotal, 0, 0, 0, 0, 0);
  } else {
    await updateAccountDailySummary(frontDeskId, 0, finalTotal, 0, 0, 0, 0);
  }
...
async function handlePrepaidPatient(req, res, frontDeskId, mobile) {
  try {
    ...
    // EXISTING PATIENT CASE
      ...
      // No registration fee for existing patient
      registrationFee = 0;

      // Check OPD fee rules
      const lastVisit = await PatientVisitRecords.findOne({
        patient: patient._id,
        doctor: doctorId,
      }).sort({ visitDate: -1 });
      ...
      if (lastVisit) {
        const daysSinceLastVisit =
          (now - lastVisit.visitDate) / (1000 * 60 * 60 * 24);

        if (daysSinceLastVisit <= 8) addOpdFee = false;
      }
    ...
    const prepaidPackage = new PrepaidPackage({
      ...
      usedVisits: 0,
      doctor: doctorId,
      department: departmentId, // IMPORTANT: department-wise restriction
      totalFee,
      paymentType,
      createdBy: frontDeskId,
    });
    ...
    await updateAccountDailySummary(frontDeskId, patientCash, patientOnline);
...
/* async function handlePrepaidPatient(req, res, frontDeskId, mobile) {
  ...
  if (!prepaidPackageId) throw new Error("Prepaid package ID is required");

  if (!validateObjectId(prepaidPackageId, res, "package id")) return;

  const packageDoc = await Package.findById(prepaidPackageId).populate(
    "services department"
  );
  if (!packageDoc) throw new Error("Invalid package selected");
  ...
  if (patient) {
    ...
    if (
      existingPackage &&
      existingPackage.doctor.department._id.toString() ===
  ...
  if (!patient) {
    ...
    patient.currentPatientType = "PREPAID";
    registrationFee = 0; // 👈
    // Check last visit to avoid OPD fee within 8 days
    const lastVisit = await PatientVisitRecords.findOne({
      patient: patient._id,
      doctor: doctorId,
    }).sort({ visitDate: -1 });
  ...
  // const invoiceItems = packageDoc.services.map((service) => {
...
/* export const usePrepaidPackage = async (req, res) => {
  try {
    ...
    // -----------------------------------------
    // GET SERVICES DIRECTLY FROM THE PACKAGE
    // -----------------------------------------
...
/* export const getAllPatients = async (req, res) => {
  try {
    ...
    const totalItems = await Patient.countDocuments({
      _id: { $in: departmentFilteredPatientIds },
      ...dateFilter,
    });
...
/* export const getAllPatientsForAdmin = async (req, res) => {
  try {
    ...
    // Fetch prepaid packages with only package info
    const prepaidPackages = await PrepaidPackage.find({
      patient: { $in: patientIds },
    }).populate("package", "name price noOfDays expireOn");
...
/* export const getPatientById = async (req, res) => {
  try {
    ...
    // Fetch patient with mobile info and lastDoctor populated
    const patient = await Patient.findOne({
      $or: [{ _id: id }, { patientId: id }],
    })
      .populate("mobile", "phone")
      .populate("lastDoctor", "doctorName"); // Populate lastDoctor

    if (!patient) return handleResponse(res, 404, "Patient not found");

    // Prepare lastDoctor snapshot
    ...
    const invoices = await Invoice.find({ patient: patient._id })
      ...
      .populate(
        "prepaidPackage",
        "packageId totalFee totalVisits usedVisits doctorSnapshot doctor"
      )
      .select(
      ...
      ); // Make sure opdCharge is selected
    ...
    //   const now = new Date();
    //   let remainingDays = Math.ceil((expiresAt - now) / (1000 * 60 * 60 * 24));
    //   let expiryMessage = "";

    //   if (remainingDays > 7)
    //     (remainingDays = 7), (expiryMessage = "7 days left");
    //   else if (remainingDays > 1) expiryMessage = `${remainingDays} days left`;
    ...
    //     opdCharge: inv.opdCharge, // Add opdCharge here
    ...
    const formattedInvoices = await Promise.all(
      invoices.map(async (inv) => {
        ...
        // 👉 Fetch usage records for this prepaid package
        let usageRecords = [];
        let remaining = [];

        // if (inv.prepaidPackage) {
        //   usageRecords = await PrepaidUsageRecord.find({
        //     prepaidPackage: inv.prepaidPackage._id,
        //   })
        //     .populate("doctor", "doctorName")
        //     .populate("servicesUsed.service", "name");

        //   // 👉 Remaining services calculation
        //   remaining =
        //     inv.prepaidPackage.services?.map((s) => ({
        //       serviceName: s.service?.name,
        //       remaining: s.quantity - s.used,
        //     })) || [];
        // }
    ...
    // Fetch all visit records for this patient
    ...
    return handleResponse(res, 200, "Patient details fetched successfully", {
      ...patientObj,
      lastDoctor: lastDoctorSnapshot, // Include populated lastDoctor
...
/* export const getTodayFrontdeskReport = async (req, res) => {
  try {
   ...
   console.log("visits===",visits);
    ...
    console.log("totalGuestRevenue===", totalGuestRevenue);
    ...
    console.log("totalCanteenRevenue====", totalCanteenRevenue);

    if (visits.length === 0) {
      return handleResponse(res, 200, "No visits found for today.", []);
    }
...
export const getTodayFrontdeskReport = async (req, res) => {
  try {
    ...
    if (visits.length === 0) {
      return handleResponse(res, 200, "No visits found for today, but here's the revenue.", [
        {
          frontdeskId: null,
          totalPatients: 0,
          newPatients: 0,
...
/* export const getAllFees = async (req, res) => {
  try {
    const opdFeeData = await OpdRegistrationFee.findOne();
    const registrationFeeData = await registrationFees.findOne();

    const response = {
      registrationFee: registrationFeeData
        ? registrationFeeData.registrationFee
        : 0,
      opdCharge: opdFeeData ? opdFeeData.registrationFee : 0,
    };

    handleResponse(res, 200, "Fees fetched successfully", response);
  } catch (error) {
...
export const getAllFees = async (req, res) => {
  try {
    ...
    // Fetch latest registration fee
    const registrationFeeData = await RegistrationFee.findOne().sort({
      updatedAt: -1,
    });
...
/* export const getPrepaidPackagesByPatient = async (req, res) => {
  try {
    ...
    const { doctorId, prepaidPackageId } = req.query;
    const { page, limit, skip } = getPagination(req);
    ...
    const formatted = await Promise.all(
      packages.map(async (pkg) => {
        ...
        return {
          ...
          doctorSnapshot: pkg.doctorSnapshot
            ? pkg.doctorSnapshot
            : pkg.doctor
            ? {
                doctorName: pkg.doctor.doctorName,
                specialization: pkg.doctor.specialization,
              }
            : null,
          ...
          usageHistory: usageRecords.map((u) => ({
            id: u._id,
            visitDate: u.visitDate,
            doctorName: u.doctor?.doctorName,
            notes: u.notes,
            servicesUsed: u.servicesUsed.map((su) => ({
    ...
    const formatted = await Promise.all(
      packages.map(async (pkg) => {
        ...
        return {
          ...
          // usageHistory: Array.isArray(usageRecords)
          //   ? usageRecords.map((u) => ({
          //       id: u._id,
          //       visitDate: u.visitDate,
          //       doctorName: u.doctor?.doctorName,
          //       notes: u.notes,
          //       servicesUsed: Array.isArray(u.servicesUsed)
          //         ? u.servicesUsed.map((su) => ({
          //             serviceName: su.service?.name,
          //             quantity: su.quantity,
          //           }))
          //         : [],
