...
/*latest running
 export const createGuest = async (req, res) => {
  try {
    ...
    const roomDoc = await GuestRoom.findById(room);
    if (!roomDoc) {
      return handleResponse(
        res,
        404,
        "Room not found with the provided Room ID"
      );
    }

    const mainBed = await Bed.findOne({ _id: mainBedId, room: roomDoc._id });
    // if (!mainBed) {
    ...
    const additionalBedIds = additionalPersons.map((p) => p.bedId);
    const beds = await Bed.find({
      _id: { $in: additionalBedIds },
      room: roomDoc._id,
    });
    ...
    if (occupiedBeds.length > 0) {
      return handleResponse(
        res,
        400,
        `These additional beds are occupied: ${occupiedBeds
          .map((b) => b._id)
          .join(", ")}`
      );
    }

    const additionalPersonsWithBeds = additionalPersons.map((p) => ({
      ...p,
      bed: p.bedId,
    }));
    ...
    // Check maximum stay limit
    if (numberOfDays > 10) {
      return handleResponse(
        res,
        400,
        "Booking cannot be made for more than 10 days"
      );
    }
    ...
    const guest = new Guest({
      ...
      additionalPersons: additionalPersonsWithBeds,
      startDate,
      endDate,
      room,
      beds: [mainBedId, ...additionalBedIds],
    ...
    for (const person of additionalPersonsWithBeds) {
      await Bed.findByIdAndUpdate(person.bed, {
        isOccupied: true,
        occupant: guest._id,
    ...
    roomDoc.availableBeds -= totalGuests;
    await roomDoc.save();

    handleResponse(res, 201, "Guest created successfully", guest);
  } catch (error) {
    console.error(error);
    handleResponse(res, 500, "Error creating guest", error.message);
  }
}; */
...
export const createGuest = async (req, res) => {
  try {
    ...
    let patientData;
    console.log("req.convertedFiles==========",req.convertedFiles);
    ...
    const startMidnight = new Date(start);
    startMidnight.setHours(0, 0, 0, 0);
    // console.log("startMidnight===", startMidnight);

    const endMidnight = new Date(end);
    endMidnight.setHours(0, 0, 0, 0);
    // console.log("endMidnight===", endMidnight);
    ...
    handleResponse(res, 201, "Guest booked successfully", guest);
  } catch (error) {
    console.error(error);
    handleResponse(res, 500, "Error creating guest", error.message);
  }
...
/* export const getGuestById = async (req, res) => {
  try {
    ...
    const guest = await Guest.findOne({ _id: id }).populate({
      path: "room",
    ...
    const mainBed = await Bed.findOne({ _id: guest.beds[0] }).select(
      "bedNumber"
    );

    const beds = await Bed.find({ _id: { $in: guest.beds } }).select(
    ...
    const additionalPersonsWithBedNumbers = guest.additionalPersons.map(
      (person) => ({
        ...person.toObject(),
        bed: bedMap[person.bed.toString()] || person.bed,
      })
    );

    const responseData = {
      ...guest.toObject(),
      mainBedNumber: mainBed ? mainBed.bedNumber : null,
      additionalPersons: additionalPersonsWithBedNumbers,
      assignedBeds: beds.map((b) => b.bedNumber),
    };
  ...
  try {
    ...
    if (search) {
      filter.$or = [
        ...
        { "room.roomType": { $regex: search, $options: "i" } },
    ...
    if (status) {
      ...
      // values: booked | checked-in | checked-out
    }
    ...
    const formattedGuests = guests.map((guest) => ({
      ...guest.toObject(),
      beds: guest.beds.map((b) => b.bedNumber),
      additionalPersons: guest.additionalPersons.map((person) => ({
        ...person.toObject(),
        bed:
          guest.beds.find((b) => b._id.toString() === person.bed.toString())
            ?.bedNumber || person.bed,
      })),
    }));
          ...
          sortRank: {
            $switch: {
              branches: [
                ...
                // 3️⃣ Checked-in & endDate ASC
                {
                  case: { $eq: ["$reservationStatus", "checked-in"] },
                  then: 3,
                },

                // 4️⃣ Booked & startDate ASC
                {
                  case: { $eq: ["$reservationStatus", "booked"] },
                  then: 4,
                },
  ...
  try {
    ...
    const roomsWithBeds = await Promise.all(
      rooms.map(async (room) => {
        ...
        const updatedAvailableBeds = availableBeds <= 0 ? 0 : availableBeds;
    ...
    const totalRooms = await GuestRoom.countDocuments({ isActive: true });
...
/* export const getAvailableRooms = async (req, res) => {
  try {
    const { page=1, limit=20, skip } = getPagination(req);
...
/* export const checkinGuest = async (req, res) => {
  try {
    ...
    const guest = await Guest.findById(guestId).populate("beds");
    ...
    const responseData = {
      ...
      beds: guest.beds.map((b) => b.bedNumber), // only bed numbers
